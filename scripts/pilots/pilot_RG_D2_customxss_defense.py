...
from prompts.xss_customapp_defense_prompts_rg import (   # RG-dedicated prompts: identical to CG except they do NOT claim the payloads form a similar cluster
...
RESULTS_DIR = Path(__file__).parent.parent / "results" / "V2" / os.environ.get("D2_OUTBASE", "Random-Groups") / ("CustomXSS_D2" + (f"_eps{CLUSTERING_EPS}" if MODE in ("clustering", "random_group") else "") + (f"_seed{RANDOM_SEED}" if os.environ.get("D2_SEED") else ""))   # random-group control: default output under results/V2/Random-Groups/ (eps- and seed-tagged)
