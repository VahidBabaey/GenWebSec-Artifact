"""
NewWorkFlow.pilot_D2_customxss_defense

Defense (D2) for the custom XSS app — mirror of the SQLi D1 defense, retargeted to XSS.
  - PAGE  in {search, calc}            -> which corpus + which /customxss endpoint/param
  - MODE  in {per_payload, clustering} -> how rules are generated

Corpus = the page's VALID + WAF-BYPASSING XSS attacks (from the A2 attack run, the winners file).
A random, non-duplicate sample (<= MAX_CORPUS) is used. Each iteration:
  1. apply all accepted rules; find which corpus attacks still BYPASS the WAF (status != 403).
  2. PER_PAYLOAD : one SecRule per unblocked attack (<=3 attempts); accepted iff it blocks the attack
                   AND keeps FP <= MAX_FP_RATE on benign traffic.
     CLUSTERING : DBSCAN-cluster the unblocked attacks, generate 1-3 GENERALISED rules per cluster,
                  accepted on the same criteria.
  3. converge when all attacks are blocked (or MAX_ITERATIONS).

Defense measures HTTP 403 through the WAF (no browser needed; validity was already established in A2).
The attack is sent to the WAF exactly as the A2 experiment sent it (same param, GET).
"""
...
from helpers.modsec_helpers import (
    write_rules, run_configtest, reload_apache, ensure_unique_rule_id, extract_secrule,
    ensure_transforms,
    rule_eval_timer, get_rule_eval_seconds, reset_rule_eval_seconds,
    modsec_error_log_offset, count_modsec_internal_failures,
)
from prompts.xss_customapp_defense_prompts import (
...
# CONFIGURATION
# ============================================================================
PAGE = os.environ.get("D2_PAGE", "calc")       # "search" | "calc"
MODE = os.environ.get("D2_MODE", "per_payload")   # "per_payload" | "clustering"

MODEL = os.environ.get("D2_MODEL", "openai/gpt-4.1-mini")
MAX_ITERATIONS = 10
MAX_ATTEMPTS = 3
MAX_FP_RATE = 0.01
# item (defense-robustness): IDENTICAL defense agent to the co-evolution pilot (C2). DECODE-only
# normalisation for XSS (NO t:lowercase: JS is case-sensitive and benign contains "print"). Enforced
# on EVERY generated rule so encoded obfuscation (%61lert, alert, &#97;) collapses before matching.
DEFENSE_TRANSFORMS = ["t:urlDecodeUni", "t:htmlEntityDecode", "t:jsDecode", "t:compressWhitespace"]
MAX_CORPUS = 300
RANDOM_SEED = int(os.environ.get("D2_SEED", "42"))   # item 1: seed for independent repeated runs (reproducible corpus sample)
...
CLUSTERING_EPS = float(os.environ.get("D2_EPS", "0.3"))   # item 6: sweepable DBSCAN threshold
CLUSTER_COVERAGE = float(os.environ.get("D2_COVERAGE", "1.0"))   # item 7: min fraction of a cluster its rules must cover
...
PAGE_CFG = {
    "search": {"file": "search.php", "param": "q",    "method": "GET", "extra": {}},
    "calc":   {"file": "calc.php",   "param": "expr", "method": "GET", "extra": {}},
}
if PAGE not in PAGE_CFG:
    raise SystemExit(f"D2_PAGE must be one of {list(PAGE_CFG)} (got {PAGE!r})")
CFG = PAGE_CFG[PAGE]
PAGE_FILE, PARAM, METHOD, EXTRA = CFG["file"], CFG["param"], CFG["method"], CFG["extra"]

CORPUS_DIR = Path(__file__).parent.parent / "results" / "V2" / "_item1_corpus"   # item 1/10: pooled union of the 5 A2 attack seeds' winners (shared by PP-Static & CG-Static)
CORPUS_FILE = CORPUS_DIR / f"A2_customxss_{PAGE}_winners.txt"

DATA_DIR = Path(__file__).parent.parent / "data"
BENIGN_FILE = DATA_DIR / "benignurls_xss.txt"

RESULTS_DIR = Path(__file__).parent.parent / "results" / "V2" / os.environ.get("D2_OUTBASE", "CustomApp_D2") / ("CustomXSS_D2" + (f"_eps{CLUSTERING_EPS}" if MODE == "clustering" else "") + (f"_seed{RANDOM_SEED}" if os.environ.get("D2_SEED") else ""))   # item 6: eps-tagged; item 1: seed-tagged; ALL D2 output nested under results/V2/CustomApp_D2/
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
LOG_FILE = RESULTS_DIR / f"D2_customxss_{PAGE}_{MODE}.txt"
RULES_FILE = RESULTS_DIR / f"D2_customxss_{PAGE}_{MODE}_rules.txt"

# advisor item 2: CRS-only benign baseline (set in main) + benign requests already blocked, so each
# iteration can report the FP ADDED over CRS and which benign requests become NEWLY blocked.
...
# ============================================================================
# corpus + benign FP corpus
# ============================================================================
def load_corpus() -> List[str]:
    if not CORPUS_FILE.exists():
        raise SystemExit(f"corpus not found: {CORPUS_FILE} (run the A2 attack agent for page={PAGE} first)")
    ...
    if len(attacks) > MAX_CORPUS:
        ...
        if _need > 0:   # corpus hygiene: drop a degenerate token and top back up to MAX_CORPUS with
                        # clean draws so the corpus stays 300. (XSS corpora are already clean -> no-op.)
...
def _build_benign_inline() -> List[Tuple[str, str, str]]:
    """Fallback benign set if data/benignurls_xss.txt is missing."""
    out: List[Tuple[str, str, str]] = []
    for q in ["shoes", "red shoes", "blue mug", "water bottle", "green tea", "coffee mug",
              "notebook", "laptop bag", "t-shirt", "phone case", "hello world", "size 10"]:
        out.append(("search", "GET", urllib.parse.urlencode({"q": q})))
    for e in ["2+2", "100*5", "(7+8)/2", "3.14*2", "1000-250", "50/2", "7*7", "12+34", "(1+2)*3"]:
        out.append(("calc", "GET", urllib.parse.urlencode({"expr": e})))
    return out


def build_benign() -> List[Tuple[str, str, str]]:
    """Load the verified-benign FP corpus (full URLs, each confirmed to PASS the baseline WAF).
    Returns (page, method, querystring) for BOTH pages, so a rule that over-blocks either page's
    normal traffic is caught. Page is recovered from the URL path."""
    out: List[Tuple[str, str, str]] = []
...
# ============================================================================
# WAF probe (same transport the A2 experiment used: GET /customxss/<page>.php)
# ============================================================================
def probe(page: str, method: str, sent: str, timeout: float = 8.0) -> Tuple[int, bool]:
    pfile = PAGE_CFG[page]["file"]
    with rule_eval_timer():     # WAF replay of attack/benign corpus = rule-eval work
        try:
            req = urllib.request.Request(f"{WAF}/customxss/{pfile}?{sent}")
...
def _fp_reject_note(attempt: int, rid: int, fp: Dict[str, object], fp_rate: float) -> str:
    ...
    return (f"attempt {attempt} id={rid}: FALSE-POSITIVED on {fp['fp']} benign request(s) "
            ...
            f"(a normal search word or arithmetic). REMOVE it; match ONLY the XSS execution structure "
            f"(a sink CALL, or the backslash-quote breakout). Do NOT match the bare word 'print' -- require print( .")
...
def generate_rules_for_cluster(cluster: List[str], cluster_id: int, used_ids: Set[int],
    ...
    log_print(f"\n  [CLUSTER {cluster_id}] size {cluster_size}: {already} covered by R_t, {len(cluster)} to cover")
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        # identical defense agent to C2: feed prior rejections (+ the benign strings they wrongly blocked)
        ...
        log_print(f"      attempt {attempt}: {len(candidate_rules)} candidate rule(s) — {desc[:80]}")
        for rule_text in candidate_rules:
            rule_text, rid = ensure_unique_rule_id(rule_text, used_ids)
            rule_text = ensure_transforms(rule_text, DEFENSE_TRANSFORMS)   # enforce decode transforms (collapses obfuscation)
            ...
            if not ok:
                log_print(f"        x syntax error: {out[:70]}")
            ...
            if marginal <= 0:
                ...
                failed_notes.append(f"attempt {attempt} id={rid}: matched 0 NEW cluster members (too narrow / wrong anchor). "
                                    f"Match the sink CALL (a sink name then `(` or backtick) or the backslash-quote breakout directly.")
                continue
    ...
    log_print(f"  [CLUSTER {cluster_id}] coverage {already}/{cluster_size} -> {cov}/{cluster_size} "
...
def generate_rule_for_payload(p: str, used_ids: Set[int], benign, failed: List[str],
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        rule_text, rid = ensure_unique_rule_id(rule_text, used_ids)
        rule_text = ensure_transforms(rule_text, DEFENSE_TRANSFORMS)   # enforce decode transforms
        ...
        if fp["errors"] > CRS_FP["errors"]:
            ...
            failed.append(f"attempt {attempt}: cumFP {fp_rate*100:.2f}% -- wrongly blocked benign e.g. {_samples}; "
                          f"remove the branch matching benign search words/arithmetic (require the sink CALL, not a bare word); rule: {rule_text}")
    return None
...
def main():
    for f in (LOG_FILE, RULES_FILE):
        if f.exists():
            f.unlink()
    log_print("=" * 80)
    log_print(f"D2 Custom-XSS Defense — page={PAGE}  mode={MODE}")
    log_print("=" * 80)
    ...
    # advisor item 2: CRS-only benign baseline (full replay, no early-stop) so every iteration can
    # report the false positives ADDED over CRS and flag benign requests NEWLY blocked by our rules.
    ...
    for it in range(MAX_ITERATIONS):
        ...
        if MODE == "per_payload":
            ...
            log_print(f"[cluster-stats eps={CLUSTERING_EPS}] clusters={n_clusters}  "
            ...
            log_print(f"DBSCAN(min_samples={CLUSTERING_MIN_SAMPLES}): {n_clusters} cluster(s) "
                      ...
                      f"{singles} singleton(s); largest sizes: {sizes[:8]}")
            for cid in sorted(clusters.keys()):
                ...
                apply_accepted_rules(accepted_rules)
        ...
        log_print(f"\nITER {it} SUMMARY: blocked={blocked_now}/{len(corpus)} ({100*blocked_now/len(corpus):.1f}%)  "
                  ...
                  f"FP={fp_rate:.2f}% ({fp['fp']}/{fp['tested']})  added {added_fp_rate:.2f}% vs CRS  "
                  f"errors {fp['errors']} ({err_rate:.2f}%)  modsecFail {mi} (added {added_mi})")
        ...
        history.append({"iter": it, "blocked": blocked_now, "rules": len(accepted_rules),
                        "clusters": n_clusters, "fp": fp_rate / 100.0,   # store as FRACTION (match D1) so a cross-family aggregator reads consistent units
    ...
    log_print(f"  wall-clock {(datetime.now() - _run_start).total_seconds():.1f}s   defense LLM calls {_ITEM10['def_calls']}   candidate rules {_ITEM10['candidates']}   accepted {_ITEM10['accepted']}   rejected {_rej10}   failed clusters {_ITEM10['failed_clusters']}")
    if MODE == "clustering" and _ITEM10['clusters_processed']:
        log_print(f"  clusters processed {_ITEM10['clusters_processed']}   total cluster attempts {_ITEM10['cluster_attempts']}   mean attempts/cluster {_ITEM10['cluster_attempts']/_ITEM10['clusters_processed']:.2f}")
