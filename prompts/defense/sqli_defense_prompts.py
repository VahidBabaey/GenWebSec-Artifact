...
You are given a CLUSTER of similar SQL-injection payloads that currently BYPASS the WAF. They belong to one of two
...
payloads in the cluster, WITHOUT blocking benign traffic.
...
- Each rule targets ONE family. If the cluster mixes families, emit one rule per family rather than one giant
  alternation.
...
_CL_USER = """Cluster #{cluster_id} contains {n} payloads that bypass the WAF:
{payload_list}
...
_CL_PREV = """YOUR PREVIOUS ATTEMPT(S) ON THIS CLUSTER FAILED — read carefully and FIX the cause, do not repeat them:
{previous_attempts_failed}
